...
export default function Table({ title, headers, initialData, from }) {
  ...
  const [currentLeaveWithdrawReason, setCurrentLeaveWithdrawReason] =
    useState("");
  ...
  const handleTabChange = (displayTab, colHeading) => {
    ...
    if (toShow === "") {
      ...
      for (let idx in initialData) {
        if (
          initialData[idx][arrpos]?.toLowerCase().includes("approved") ||
          initialData[idx][arrpos]?.toLowerCase().includes("disapproved")
        ) {
  ...
  async function handleDeleteLeaveApplication(leaveID) {
    if (
      ...
      currentLeaveWithdrawReason.length == 0
    ) {
      toast.error(
        "Withdraw reason must be mentioned",
    ...
    const resp = await httpClient.post(
      ...
      {
        leave_id: leaveID,
        reason: currentLeaveWithdrawReason,
      }
    );
            ...
            <FaIcons.FaEye
              ...
              onClick={(e) => {
                if (position.includes("pg") == false)
                  navigate(
                    `/${from}/${
                      row[1].toLowerCase().startsWith("casual")
                        ? "casual"
                        : "non_casual"
                    }/${row[0]}`
                  );
                else navigate(`/${from}/${"pg_applications"}/${row[0]}`);
              }}
            />
            {row[5].includes("Pending") || row[5].includes("Disapproved") ? (
                        <FaIcons.FaEdit
                            style={{ cursor: "pointer", marginRight: "5px" }}
                            color="blue"
                            onClick={(e) => {
                                if (position.includes("pg") == false)
                                    navigate(
                                        `/${from}/edit_leave/${row[0]}`
                                    );
                                else navigate(`/${from}/${"editpg_application"}/${row[0]}`);
                            }}
                        />
                    ) : null}
    ...
    <div className="container ">
      <Modal show={showConfirmDeleteAction} onHide={handleClose}>
        ...
        <Modal.Body>
          ...
          {currentLeaveStatus?.toLowerCase().includes("approved") &&
          currentLeaveStatus?.toLowerCase().includes("disapproved") == false ? (
            <div>
              <br />
              <p>Enter the reason for withdraw:</p>
              <textarea
                onChange={(e) => {
                  setCurrentLeaveWithdrawReason(e.target.value);
                }}
              ></textarea>
            </div>
          ) : (
                    ...
                    <tr key={idx} className="cell-1">
                      {row.map((item, i) => {
                        if (
                          ...
                          String(item).toLowerCase().startsWith("approved")
                        ) {
                          return (
                            <td key={i}>
                          ...
                          return (
                            <td>
                              ...
                              <button
                                ...
                                title={`Reason - ${item}`}
                                onClick={async () => {
                                  await disapproveWithdraw(row[0]);
                                }}
                              >
