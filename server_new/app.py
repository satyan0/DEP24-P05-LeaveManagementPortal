	...
	try:
		...
		for i in data:
			...
			for col, val in zip(columns, i):
				if col in ['file_data', 'signature', 'advisor', 'ta_instructor']:
...
def get_leave_info_by_id():
	try:
		# if (not session.get('user_info') or not check_user(session.get('user_info')['email'])):
		# 	print(session.get('user_info'))
		# 	print("forbidden: get_leave_info_by_id")
		# 	return get_error_response("Forbidden")
		...
		for i in leaves:
			...
			# 	print(col)
			# 	print(val)
				if col in ['file_data', 'signature','advisor','ta_instructor'] or "_sig" in col:
					# print(f"Size of {col}: {sys.getsizeof(val)} bytes")
					# if(col=='signature'):
					# 	col.decode("utf-8")
					# 	content[col] = val
					continue
				content[col] = val
				# print(f"Size of {col}: {sys.getsizeof(val)} bytes")
			# content['signature'] = base64.b64encode(i['signature']).decode('utf-8')
...
def fetch_remaining_leaves():
	try:
		...
		if (not session.get('user_info') or not check_user(session.get('user_info')['email'])):
			print("forbidden: fetch_remaining_leaves")
			return get_error_response("Forbidden")
		email = session['user_info']['email']
		data = get_user_data(email)[0]
		user_id = data[0]
